...
train_or = read_csv("Datasets/traffic_forecasting/processed_train.csv", index_col="Datetime", parse_dates=True)
test_or = read_csv("Datasets/traffic_forecasting/processed_test.csv", index_col="Datetime", parse_dates=True)

train: Series = train_or[["Total"]].values.astype("float32")
test: Series = test_or[["Total"]].values.astype("float32")
...
plot_forecasting_eval(
    ...
    prd_trn,
    prd_tst,
    title=f"LSTM (length={best_length}, hidden={params[1]}, epochs={params[2]})",
)
savefig(f"Lab6/results/LSTM/evals.png")

train, test = train_or[["Total"]], test_or[["Total"]]
...
savefig(f"Lab6/results/LSTM/forecast.png")